...
def main():
    ...
    parser.add_argument('plotgeom', type=str, help='plotgeom file name')
    ...
    if not os.path.isfile(args.plotgeom):
        sys.exit("plotgeom2root: File %s does not exist." % args.plotgeom)

    wlength = 0
    title = ""

    if args.root == "":
        rootFileName = "%s%s" % (args.plotgeom,".root")
    else:
...
#                print(X0,Y0,Z0,X1,Y1,Z1,TYX,TYY,TYZ,TXX,TXY,TXZ,XAXLEN,YAXLEN)
        ...
        i = 0
        fout = ROOT.TFile(rootFileName, "recreate", args.plotgeom)
